...
class linedodger:
    class Player:
        ...
        def update(self, lines):
            ...
            if pressedkeys[pygame.K_LEFT] and pressedkeys[pygame.K_RIGHT]:
                ...
                pygame.K_RIGHT
            ]:  # moves the player right if the left arrow is pressed
                self.x += self.vel
            if self.x < self.size:
                self.x = self.size
        ...
        def __init__(self, SCREEN, SCREEN_SIZE, speed):
            ...
            for _ in range(number_of_holes):
                while True:
                    ...
                    if (
                        chosen_one not in self.holepositions
                        and (chosen_one - 1) not in self.holepositions
                        and (chosen_one + 1) not in self.holepositions
                    ):
    ...
    class Background:
        def __init__(self):
            self.objects = []
            self.star = pygame.image.load(
                path.join("assets", "star.png")
            ).convert_alpha()
            self.galaxy = pygame.image.load(
                path.join("assets", "galaxy.png")
            ).convert_alpha()
        ...
        def update(self, SCREEN, WIDTH, HEIGHT, speed):
            ...
            SCREEN.blit(
                self.galaxy,
                (
                    (WIDTH - self.galaxy.get_width()) / 2,
                    (HEIGHT - self.galaxy.get_height()) / 2,
                ),
            )
            if len(self.objects) != self.max_objects:
                ...
                if spawn == self.spawn_chance:
                    ...
                    image = pygame.transform.scale(
                        self.star, (new_object_size, new_object_size)
                    )
                    self.objects.append(
                        {
                            "object": image,
            ...
            for flying_object in self.objects:
                if flying_object["y"] > HEIGHT:
                    ...
                    SCREEN.blit(
                        flying_object["object"],
                        (flying_object["x"], flying_object["y"]),
                    )
        ...
        if self.started:
            ...
            for line in self.lines:
                if line.update() == -1:
                    ...
                    del line
                    self.speed += 0.1
            display_score = self.font.render(
                str(self.player.score), False, (255, 255, 255)
            )
            self.SCREEN.blit(
                display_score, (self.WIDTH - 64 * len(str(self.player.score)), 20)
            )
        else:
            display_start = self.font.render(
                "Press Space to Start", False, (255, 255, 255)
            )
            self.SCREEN.blit(
                display_start,
                (
                    (self.WIDTH - (30 * len("Press Space to Start"))) / 2,
                    (self.HEIGHT / 2 - 32),
                ),
            )
    ...
    def run(self):
        ...
        while True:
            ...
            if_error = (
                self.updateWindow()
            )  # updates the window with the new player position
            if if_error == -1:
                return -1
            for (
                event
                ...
                pygame.event.get()
            ):  # checks if you close the window and then stops the game
                if event.type == pygame.QUIT:
                    del self.player
                    return -2
                if event.type == pygame.KEYDOWN:
    ...
    def main_loop(self, WIDTH, HEIGHT):
        ...
        self.bg = self.Background()  # BACKGROUND
        ...
        pygame.mixer.music.load(
            path.join("assets", "space_music.wav")
